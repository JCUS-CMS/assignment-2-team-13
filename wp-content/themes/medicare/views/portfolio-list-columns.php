<?php
	...
	$share_html = '<div class="btIconRow">' . ( function_exists( 'boldthemes_get_share_html' ) ? boldthemes_get_share_html( $permalink, 'pf', 'btIcoExtraSmallSize', 'btIcoDefaultType btIcoDefaultColor' ) : '' ) . '</div>';
	...
	if ( $blog_author || $blog_date || $show_comments_number ) {
		$meta_html .= '';
		if ( $blog_date && ! $blog_side_info ) $meta_html .= '<span class="btArticleDate">' . esc_html( date_i18n( MedicareTheme::$boldthemes_date_format, strtotime( get_the_time( 'Y-m-d' ) ) ) ) . ' </span>'; 
	...
	echo '<article class="' . esc_attr( implode( ' ', get_post_class( $class_array ) ) ) . ' btBlogColumnView">';
		echo '<div class="port">';
			echo '<div class="boldCell">';
				echo '<div class = "boldRow bottomExtraSmallSpaced">';
				if($media_html != "") {
					echo '<div class="rowItem ' . esc_attr( $left_col_class ) . ' btTextCenter">' . ( $media_html ) . '</div>';
				}
					...
					echo '<div class="rowItem btTextLeft ' . esc_attr( $right_col_class ) . '">';
						echo '<div class="rowItemContent">';
							...
							if ( $blog_author && ! $blog_side_info ) echo '<p class="btArticleListBodyAuthor">' . wp_kses_post( $author_html ) . '</p>' . wp_kses_post( $share_html );
						echo '</div>' ;
					echo '</div>';
				...
				if ( $blog_side_info && ! is_search() ) {
					...
					if ( $blog_date ) {
						echo '<div class="asgItem date"><small>' . esc_html( $blog_date_side ) . '</small></div>';
					}
