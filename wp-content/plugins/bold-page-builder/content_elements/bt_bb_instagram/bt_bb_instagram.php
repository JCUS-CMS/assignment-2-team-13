...
class bt_bb_instagram extends BT_BB_Element {
	...
	function handle_shortcode( $atts, $content ) {
		...
		$output = '<div' . $id_attr . ' class="' . implode( ' ', $class ) . '"' . $style_attr . '>';
			ob_start();
			the_widget( 'BB_Instagram', array( 'username' => $username, 'number' => $number, 'hashtag' => $hashtag, 'cache' => $cache ) );
	...
	function map_shortcode() {
		...
		bt_bb_map( $this->shortcode, array( 'name' => esc_html__( 'Instagram', 'bold-builder' ), 'description' => esc_html__( 'Recent Instagram images', 'bold-builder' ), 'icon' => $this->prefix_backend . 'icon' . '_' . $this->shortcode,
			'params' => array(
